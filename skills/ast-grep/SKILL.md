...
The `ast-grep` CLI must be installed. Check with `ast-grep --version`; if missing, install it:
...
The binary is invoked as `ast-grep` (an `sg` alias also exists, but `ast-grep` avoids collisions with the system `sg`).
