...
2. **Composition payoff** — Given the abstractions below it, is each unit nearly trivial? If a package is fat, the abstractions under it are wrong. Config should be methods/fields on the object; required dependencies positional constructor args — no functional-options or config-struct boilerplate for one caller.
