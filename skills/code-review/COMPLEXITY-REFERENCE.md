...
SlopCodeBench measured the same effect rather than inferring it. Its two metrics — verbosity, the share of lines that are clones or slop-rule hits, and erosion, the share of function mass (complexity × √lines) sitting in functions over complexity 10 — put established repos at roughly 0.15 and 0.31 and agent output at roughly 0.33 and 0.68, about double on both. The benchmark's structure is the more important finding: it feeds specifications in rounds and erases the agent's context between them, mimicking real use, and under the strict criterion (every checkpoint's tests pass) every model tested scored 0%. Bad decisions compound because nothing remembers them. The reviewer is in the same position — it sees one diff with no memory of the last — which is why the gate asks for the landing zone's measured state and not just the change's shape. The same source warns that an LLM asked to rate code quality is close to a random number generator and that pairwise judgments flip when the candidates are renamed; the metrics exist because taste doesn't transfer to the judge, and the article is explicit that the metrics still bottom out in hand-written rules. Use them to locate, not to conclude.
...
- Gabriel Orlanski et al., [SlopCodeBench](https://arxiv.org/abs/2603.24755) (2026) and its CLI [scb-check](https://github.com/gabeorlanski/scb-check) — verbosity and erosion; context-erased iterative evaluation. Sebastian (Earendil), [Measuring the sloppiness of code](https://earendil.com/posts/measuring-code-sloppiness/) (2026-09) — why LLM-as-judge fails on quality, LOC delta as a Goodhart-prone signal, the roughly 2× agent-versus-repo gap.
